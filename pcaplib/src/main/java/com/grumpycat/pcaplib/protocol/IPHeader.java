...
public class IPHeader {
	...
	private static final byte offset_ver_ihl = 0; //0: 版本号（4bits） + 首部长度（4bits）
    private static final byte offset_tos = 1; //1：服务类型偏移
    private static final short offset_tlen = 2; //2：总长度偏移
    private static final short offset_identification = 4; //4：16位标识符偏移
    private static final short offset_flags_fo = 6; //6：标志（3bits）+ 片偏移（13bits）
    private static final byte offset_ttl = 8; //8：生存时间偏移
    private static final short offset_crc = 10; //10：首部校验和偏移
    private static final int offset_op_pad = 20; //20：选项 + 填充
	...
	public String toString() {
		return String.format(Const.LOCALE, "%s->%s Pro=%s, HLen=%d", CommonMethods.ipIntToString(getSourceIP()),
